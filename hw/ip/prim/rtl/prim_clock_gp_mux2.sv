...
generate
  ...
  for (i = 0; i < 2; i = i++) begin: gen_two_flops
    always_ff @(posedge clk_gp[i] or negedge rst_n) begin: stage1
      if (!rst_n) begin
        intq[i] <= 1'b0;
      end else begin
        intq[i] <= stage_d[i];
    ...
    always_ff @(negedge clk_gp[i] or negedge rst_n) begin: stage2
      if (!rst_n) begin
        stage_q[i] <= 1'b0;
      end else begin
        stage_q[i] <= intq[i];
