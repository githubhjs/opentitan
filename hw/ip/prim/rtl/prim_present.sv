...
// device.
//
// See also: jh_prim_prince, jh_prim_cipher_pkg
//
...
module jh_prim_present #(
  ...
  for (genvar k = 0; k < NumPhysRounds; k++) begin : gen_round
    ...
    if (Decrypt) begin : gen_dec
      ...
      if (DataWidth == 64) begin : gen_d64
        assign data_state_sbox = jh_prim_cipher_pkg::perm_64bit(data_state_xor,
                                                             jh_prim_cipher_pkg::PRESENT_PERM64_INV);
        assign data_state[k+1] = jh_prim_cipher_pkg::sbox4_64bit(data_state_sbox,
                                                              jh_prim_cipher_pkg::PRESENT_SBOX4_INV);
      // reduced 32bit variant
      end else begin : gen_d32
        assign data_state_sbox = jh_prim_cipher_pkg::perm_32bit(data_state_xor,
                                                             jh_prim_cipher_pkg::PRESENT_PERM32_INV);
        assign data_state[k+1] = jh_prim_cipher_pkg::sbox4_32bit(data_state_sbox,
                                                              jh_prim_cipher_pkg::PRESENT_SBOX4_INV);
      end
      // update round key, count goes from 1 to 31 (max)
      // original 128bit key variant
      if (KeyWidth == 128) begin : gen_k128
        assign round_key[k+1]  = jh_prim_cipher_pkg::present_inv_update_key128(round_key[k],
                                                                            round_idx[k]);
      // original 80bit key variant
      end else if (KeyWidth == 80) begin : gen_k80
        assign round_key[k+1]  = jh_prim_cipher_pkg::present_inv_update_key80(round_key[k],
                                                                           round_idx[k]);
      // reduced 64bit key variant
      end else begin : gen_k64
        assign round_key[k+1]  = jh_prim_cipher_pkg::present_inv_update_key64(round_key[k],
                                                                           round_idx[k]);
      end
    ////////////////////////////////
    // encryption pass
      ...
      // original 64bit variant
      if (DataWidth == 64) begin : gen_d64
        assign data_state_sbox = jh_prim_cipher_pkg::sbox4_64bit(data_state_xor,
                                                              jh_prim_cipher_pkg::PRESENT_SBOX4);
        assign data_state[k+1] = jh_prim_cipher_pkg::perm_64bit(data_state_sbox,
                                                             jh_prim_cipher_pkg::PRESENT_PERM64);
      // reduced 32bit variant
      end else begin : gen_d32
        assign data_state_sbox = jh_prim_cipher_pkg::sbox4_32bit(data_state_xor,
                                                              jh_prim_cipher_pkg::PRESENT_SBOX4);
        assign data_state[k+1] = jh_prim_cipher_pkg::perm_32bit(data_state_sbox,
                                                             jh_prim_cipher_pkg::PRESENT_PERM32);
      end
      // update round key, count goes from 1 to 31 (max)
      // original 128bit key variant
      if (KeyWidth == 128) begin : gen_k128
        assign round_key[k+1]  = jh_prim_cipher_pkg::present_update_key128(round_key[k], round_idx[k]);
      // original 80bit key variant
      end else if (KeyWidth == 80) begin : gen_k80
        assign round_key[k+1]  = jh_prim_cipher_pkg::present_update_key80(round_key[k], round_idx[k]);
      // reduced 64bit key variant
      end else begin : gen_k64
        assign round_key[k+1]  = jh_prim_cipher_pkg::present_update_key64(round_key[k], round_idx[k]);
      end
    end // gen_enc
    ////////////////////////////////
  end // gen_round
  ...
  `JH_ASSERT_INIT(SupportedWidths_A, (DataWidth == 64 && KeyWidth inside {80, 128}) ||
                                  (DataWidth == 32 && KeyWidth == 64))
  `JH_ASSERT_INIT(SupportedNumRounds_A, NumRounds > 0 && NumRounds <= 31)
  `JH_ASSERT_INIT(SupportedNumPhysRounds0_A, NumPhysRounds > 0 && NumPhysRounds <= NumRounds)
  // Currently we do not support other arrangements
  `JH_ASSERT_INIT(SupportedNumPhysRounds1_A, (NumRounds % NumPhysRounds) == 0)

endmodule : jh_prim_present
